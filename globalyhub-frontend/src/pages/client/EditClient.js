...
import { editClient, getClientById } from "../../redux/thunk/clientThunk";
import { errorToast, successToast } from "../../utils/toastify";

import { resetDataById } from "../../redux/slice/clientSlice";
...
const EditClient = () => {
  ...
  useEffect(()=>{
    if(client?.dataById){
setFormFieldData({
  name: client?.dataById?.name,
    gender: client?.dataById?.gender,
    email: client?.dataById?.email,
    phone: client?.dataById?.phone,
    address: client?.dataById?.address,
    nationality: client?.dataById?.nationality,
    date_of_birth: client?.dataById?.date_of_birth,
    education_background: client?.dataById?.education_background,
    preferred_contact: client?.dataById?.preferred_contact,
})
    }
  },[client.dataById])
  // const { loading } = useSelector((state) => state.clientState);
  ...
  const handleSubmit = async (event) => {
    ...
    try {
      ...
      if (formFieldData && id) {
        dispatch(editClient({ data: formFieldData,id:id, callback: successAdd }));
      }
    } catch (err) {}
  };
  const successAdd = () => {
    successToast("Client Edited Successfully");
    navigate("/");
  };
